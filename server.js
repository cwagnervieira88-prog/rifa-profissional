...
async function initDatabase() {
  ...
  if (rifas.length === 0) {
    await db.run(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `, 'Rifa iPhone 15', 'https://images.unsplash.com/photo-1606813907291-d86efa9b94db?w=400', 100, 15.00, 'Sorteio dia 30/12! Prêmio incrível!', 'exemplo@email.com');
...
app.post('/api/pagamento/pix', async (req, res) => {
  ...
  try {
    ...
    await db.run(
      'INSERT INTO pagamentos (id, rifa_id, numeros, valor_total, status) VALUES (?, ?, ?, ?, ?)',
      pagamentoId, rifa_id, numerosStr, valor_total, 'pendente'
    );
    
    const qrCodeBase64 = await QRCode.toDataURL(`pix:teste?valor=${valor_total}&id=${pagamentoId}`);
    
    res.json({
      pagamento_id: pagamentoId,
      qr_code: qrCodeBase64,
      pix_copia_cola: `00020126580014BR.GOV.BCB.PIX0136${pagamentoId}@rifa.com5204000053039865404${valor_total.toFixed(2)}5802BR5925Rifa Sistema6009SAO PAULO62070503***6304E3A9`,
...
app.post('/api/confirmar-pagamento', async (req, res) => {
  const { pagamento_id, nome_cliente, telefone } = req.body;
  
  try {
    const pagamento = await db.get('SELECT * FROM pagamentos WHERE id = ?', pagamento_id);
    if (!pagamento) return res.status(404).json({ error: 'Pagamento não encontrado' });
    
    const numeros = pagamento.numeros.split(',').map(Number);
    
    for (let numero of numeros) {
      await db.run(
        'INSERT INTO numeros_vendidos (rifa_id, numero, nome_cliente, telefone, pagamento_id) VALUES (?, ?, ?, ?, ?)',
        pagamento.rifa_id, numero, nome_cliente, telefone, pagamento_id
      );
    }
    
    await db.run('UPDATE pagamentos SET status = "pago" WHERE id = ?', pagamento_id);
